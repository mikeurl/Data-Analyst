...
1. Compare the ORIGINAL USER QUESTION to the SQL results
2. If the user asked for specific fields/attributes that don't exist in the results, EXPLICITLY acknowledge this
3. Example: User asks "retention by country of origin" but SQL only has gender/race → Start with "Note: This database doesn't track country of origin. However, here are retention rates by the demographics we do track..."
4. Always note what fields ARE available if requested fields are missing
...
**Note:** This database doesn't track country of origin. However, here are retention rates by the demographics we do track:
...
CRITICAL - HANDLING MISSING DATA FIELDS:
1. Compare the ORIGINAL USER QUESTION to what's in the SQL/Python results
2. If the user asked for specific fields that aren't in the database, EXPLICITLY note this upfront
3. Example: User asks "predictors including socioeconomic status" but that field doesn't exist → Start with "Note: While socioeconomic status isn't tracked in this database, the regression analyzed these available factors..."
4. Always clarify what data WAS used if requested data is missing
...
**Note:** While country of origin isn't tracked in this database, the regression used these available demographic factors: gender, race/ethnicity, and program enrollment.

[Table with regression results using available fields]
